...
namespace ByteCoffee.Utility.Helpers
{
    public static class SpellHelper
    {
        ...
        public static string GetInitials(string text)
        {
            ...
            foreach (var py in text.Select(GetSpell).Where(py => py != ""))
            { chars.Append(py[0]); }
        ...
        {
            var temp = ConvertEncoding(text, encoding, Encoding.UTF8);
        ...
        public static string GetSpell(string text)
        {
            ...
            foreach (var py in text.Select(GetSpell).Where(py => py != ""))
            { sbPinyin.Append(py); }//sbPinyin.Append(" ");
        ...
        public static string GetSpell(string text, Encoding encoding)
        {
            var temp = ConvertEncoding(text.Trim(), encoding, Encoding.UTF8);
            return ConvertEncoding(GetSpell(temp), Encoding.UTF8, encoding);
        }
        ...
        public static string GetChineseText(string pinyin)
        {
            ...
            foreach (var str in PyCode.Codes.Where(str => str.StartsWith(key + " ") || str.StartsWith(key + ":")))
        ...
        {
            var text = ConvertEncoding(pinyin, encoding, Encoding.UTF8);
        ...
        public static string GetSpell(char ch)
        {
            var hash = GetHashIndex(ch);
            for (var i = 0; i < PyHash.Hashes[hash].Length; ++i)
            {
                var index = PyHash.Hashes[hash][i];
        ...
        public static string GetSpell(char ch, Encoding encoding)
        {
            ch = ConvertEncoding(ch.ToString(), encoding, Encoding.UTF8)[0];
            return ConvertEncoding(GetSpell(ch), Encoding.UTF8, encoding);
        }
        ...
        {
            var srcBytes = srcEncoding.GetBytes(text);
    ...
    {
        internal static string[] Codes = new[]{
"a     :阿啊吖嗄腌锕",
...
"zuo   :作做左座坐昨佐柞阼唑嘬怍胙祚"};
    ...
    {
        internal static short[][] Hashes = new[]{
